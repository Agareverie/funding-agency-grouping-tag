...
for (const folder of folders){
    ...
    for (const file of files){
        ...
        const data = JSON.parse(fs.readFileSync(filePath, "utf8"))["abstracts-retrieval-response"];
        output.push(data);
        progress++;
    }

    fs.writeFileSync(path.join(rawPath,`${folder}.json`), JSON.stringify(output), "utf8");
}
