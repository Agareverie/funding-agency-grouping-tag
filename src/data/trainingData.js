...
const grants = JSON.parse(fs.readFileSync(path.join(resourcePath, "processed", "grants_unfiltered.json")));

//a tag will need to appear in atleast (threshold) grants to be considered (all tags pass this criteria so this can now be safely removed)
const threshold = 50;
...
for(const grant of Object.values(grants)){
    ...
    for(const paperId of grant.papers){
        ...
        for (const group of Object.keys(paper.subjectAreas)){
            ...
            if(countFrequencies) groupFrequencies[group] += paper.subjectAreas[group].length;
    ...
    for(const group of Object.keys(groupFrequencies)){
        if(interGrantGroupFrequencies[group] == null) interGrantGroupFrequencies[group] = 0;
        interGrantGroupFrequencies[group]++;
    }

    output.push({id: grant.id,name: grant.name, groupFrequencies: groupFrequencies})
}

//sort used tags and filter out non overlapping tags
usedGroups = [...usedGroups].filter(x=>interGrantGroupFrequencies[x] >= threshold).toSorted();
...
fs.writeFileSync(path.join(resourcePath, "processed", "grant_data.csv"), csvOutput)