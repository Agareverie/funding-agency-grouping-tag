...
const ids = JSON.parse(fs.readFileSync(path.join(basePath,"resources/data/processed/unique_author_ids.json")));
...
async function verify(id) {
    fetch(`https://www.scopus.com/authid/detail.uri?authorId=${id}`, {
        ...
        const title = document.getElementsByTagName("title")[0].innerHTML.trim();
        ...
        else if(title != "Scopus - Page not found") verified.push(id);
...
function next(){
    if(ids.length > 0) setTimeout(()=>{verify(ids.pop())},Math.random()*maxWait);
    else{
